...
def main():
    ...
    for pid in pid_unique:
        ...
        meanscores = [np.mean(class1_scores), np.mean(class2_scores)]
        if meanscores[1] > threshold:
            predlabel = 1
        else:
            predlabel = 0

        if truelabel != predlabel:
            wrongs[pid] = [np.max(meanscores), truelabel]
        else:
            rights[pid] = [np.max(meanscores), truelabel]
    ...
    # Write output to csv
    output = args.svm.replace('.hdf5', '_probs.csv')
