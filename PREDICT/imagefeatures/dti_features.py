...
def get_dti_features(image, mask, meta_data):
    ...
    panda_labels = ['ADC_mean', 'ADC_std', 'ADC_min', 'ADC_max']

    pandas_dict = dict(zip(panda_labels, dti_features))
    dti_features = pd.Series(pandas_dict)
...
def get_dti_post_features(image, mask, meta_data):
    ...
    panda_dict = dict(zip(panda_labels, dti_features))

    dti_features = pd.Series(panda_dict)
    return dti_features
