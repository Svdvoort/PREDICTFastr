...
def RankSVM_train_old(train_data, train_target, cost=1, lambda_tol=1e-6,
    ...
    for i in range(0, n_object):
        if np.sum(train_target[:, i]) != -n_class + 2:
            raise PREDICTexceptions.PREDICTIOError('RankedSVM only works ' +
...
def RankSVM_train(train_data, train_target, cost=1, lambda_tol=1e-6,
    ...
    while(continuing):
        ...
        try:
            ...
            print('[PREDICT Warning] RankedSVM could not be fit to data. Returning zero classifier.')
            Alpha_new = Alpha
    ...
    if Bias.shape == ():
        ...
        print('[PREDICT Warning] Error in Alpha, create empty bias.')
