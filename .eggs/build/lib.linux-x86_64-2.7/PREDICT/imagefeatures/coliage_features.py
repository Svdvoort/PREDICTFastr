...
def get_coliage_features(image, mask):
    ...
    panda_dict = dict(zip(coliage_labels, coliage_features))

    coliage_dict = dict()
    coliage_dict['all'] = pd.Series(panda_dict)

    coliage_features = pd.Series(coliage_dict)

    return coliage_features
